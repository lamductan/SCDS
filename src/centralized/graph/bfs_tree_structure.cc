...
centralized::BFSTreeStructure::BFSTreeStructure(int nodeid, int level,
    ...
    : nodeid(nodeid), level(level), parent(parent), children(children)
{
}
...
std::string centralized::BFSTreeStructure::to_string(int log_level) const
{
    std::string one_tab = std::string(4, ' ');
    std::string tab = std::string(log_level * 4, ' ');
    std::string s = tab + "BFSTreeStructure = {\n";
