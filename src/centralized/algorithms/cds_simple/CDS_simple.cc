...
centralized::CDSSimpleAlg::CDSSimpleAlg(
    std::map<int, centralized::Node *> nodes, std::set<centralized::Edge> edges)
{
    this->graph = new Graph(nodes, edges, CDS_SIMPLE_NODE_TYPE);
}

bool centralized::CDSSimpleAlg::check_all_nodes_decided_mis_status(
    bool print_undediced_node)
{
...
std::vector<int> centralized::CDSSimpleAlg::find_cds(bool apply_rule)
{
...
void centralized::CDSSimpleAlg::find_three_hop_neighbors()
{
    ...
    graph->find_three_hop_neighbors();
    for (auto it : graph->nodes) {
        Node *node = it.second;
        CDSSimpleNode *cds_simple_node = dynamic_cast<CDSSimpleNode *>(node);
        // std::cout << "\tnode" << node->id << '\n';
        cds_simple_node->two_three_hop_neighbors =
            combine_map<Node *, std::array<Node *, 2>>(
                node->two_hop_neighbors, node->three_hop_neighbors);
    }
}

std::set<int> centralized::CDSSimpleAlg::find_cds_nodes_from_mis_nodes(
...
std::set<int>
centralized::CDSSimpleAlg::apply_rule_1(const std::set<int> &cds_nodes)
{
    ...
    for (int u : cds_nodes_1) {
        ...
        for (int v : node_u->neighbors_set) {
            ...
            // else if (cond2) std::cout << "\t" << "Rule1 condition 2
            // satisfied\n";
...
std::set<int>
centralized::CDSSimpleAlg::apply_rule_2(const std::set<int> &cds_nodes)
{
    ...
    for (int u : cds_nodes_1) {
        ...
        for (int v : node_u->neighbors_set) {
            ...
            for (int w : node_u->neighbors_set) {
                ...
                // if (cond1) std::cout << "\t" << "Rule2 condition 1
                // satisfied\n"; else if (cond2) std::cout << "\t" << "Rule2
...
centralized::CDSSimpleAlg::~CDSSimpleAlg() { delete graph; }