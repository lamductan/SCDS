...
centralized::WAF02CDSAlg::WAF02CDSAlg(std::map<int, centralized::Node *> nodes, std::set<centralized::Edge> edges) {
...
std::map<int, centralized::BFSTreeStructure> centralized::WAF02CDSAlg::find_bfs_tree_structure_from_root(int root) {
    std::cout << "centralized::WAF02CDSAlg::find_bfs_tree_structure_from_root()\n";
    ...
    while (!q.empty()) {
        ...
        if (parent != -1) bfs_tree_structure_all_nodes[parent].children.insert(u);
    ...
    for(auto it : WAF02_cds_nodes) {
        ...
        WAF02_cds_node->bfs_tree_structure = bfs_tree_structure_all_nodes[nodeid];
    }
    ...
    for(auto it : WAF02_cds_nodes) {
        ...
        for(auto it1 : it.second->neighbors) {
            ...
            WAF02CDSNode *WAF02_cds_neighbor_node = WAF02_cds_nodes[neighbor_id];
            int l1 = WAF02_cds_neighbor_node->bfs_tree_structure.level;
            if (l1 > l || (l1 == l && neighbor_id > nodeid)) --WAF02_cds_node->y;
            //std::cout << "\t\t\t" << "neighbor_id = " << neighbor_id << ", l1 = " << l1 << ", y = "
            //          << WAF02_cds_node->y << '\n';
        }
    }
...
std::vector<int> centralized::WAF02CDSAlg::find_mis(bool print_undediced_node) {
    ...
    while (!order.empty()) {
        ...
        //std::cout << "\t" << "select node" << nodeid << ", level = " << lowest_rank.first << '\n';
...
std::set<int> centralized::WAF02CDSAlg::find_cds_nodes_from_mis_nodes(const std::vector<int> &mis_nodes) {
...
int centralized::WAF02CDSAlg::find_root_T_star(int root) {
    ...
    for(auto it : root_node->neighbors) {
        ...
        //std::cout << "\t" << "neighbor_id = " << neighbor_id << ", " << "n_mis_neighbors = " << n_mis_neighbors << '\n';
