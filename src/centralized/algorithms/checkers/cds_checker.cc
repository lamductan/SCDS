...
centralized::CDSChecker::CDSChecker(CDSSimpleAlg *alg, const std::vector<int> &selected_nodes)
...
bool centralized::CDSChecker::check() const {
    ...
    std::cout << "PASS CENTRALIZED CDS CHECK! There are " << selected_nodes.size() << " nodes selected.\n";
    return true;
}