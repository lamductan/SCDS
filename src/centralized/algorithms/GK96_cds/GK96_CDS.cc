...
centralized::GK96CDSAlg::GK96CDSAlg(std::map<int, centralized::Node *> nodes, std::set<centralized::Edge> edges) {
    this->graph = new Graph(nodes, edges, CDS_GK96_NODE_TYPE);
}

std::set<int> centralized::GK96CDSAlg::find_cds_nodes_from_mis_nodes(const std::vector<int> &mis_nodes) {
...
std::vector<centralized::Edge> centralized::GK96CDSAlg::find_cluster_edges(std::set<int> &mis_nodes_set) {
    ...
    for(Edge edge : graph->edges) {
        ...
        if (cluster_u == cluster_v) continue;
        ...
        if (u == cluster_u) {
            ...
            cluster_edge.nodes_on_edge.insert(u);
        }

        if (v == cluster_v) {
            --cluster_edge.w;
        } else {
            cluster_edge.nodes_on_edge.insert(v);
        }

        //std::cout << '\t' << "u = " << u << ", v = " << v << ", cluster_edge = " << cluster_edge.to_string() << '\n';
        if (cluster_edge.w == 0) continue;
        
        std::pair<int,int> p = {cluster_u, cluster_v};
        if (cluster_edges_map.find(p) == cluster_edges_map.end()) {
            cluster_edges_map[p] = cluster_edge;
        } else {
            if (cluster_edge.w == 2) continue;
            if (cluster_edges_map[p].w == 1) continue;
            else cluster_edges_map[p] = cluster_edge;
        }
    }
    for(auto it : cluster_edges_map) {
        std::pair<int,int> p = it.first;
        Edge e = it.second;
        //std::cout << "\tcluster_edges_map[" << p.first << "," << p.second << "] = " << e.to_string() << '\n';
    }

    return getMapValues<std::pair<int,int>, Edge>(cluster_edges_map);
}

std::vector<centralized::Edge> centralized::GK96CDSAlg::build_mst(std::vector<centralized::Edge> &edges) {
