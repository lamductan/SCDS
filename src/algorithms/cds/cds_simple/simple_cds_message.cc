...
SimpleCDSMessage::SimpleCDSMessage(const char* name, short kind) : cMessage(name, kind) {}
...
SimpleCDSMessage::SimpleCDSMessage(const SimpleCDSMessage& other) : cMessage((cMessage) other) {
...
void SimpleCDSMessage::setMISInfo(const std::map<int, std::array<int,3>> &MIS_info) {
    this->MIS_info = MIS_info;
}

std::set<int> SimpleCDSMessage::getCDSNodes() const {
    return cds_nodes;
}
...
std::string SimpleCDSMessage::to_string(int n_tab) const {
    std::string tab = std::string("    ", n_tab);
    std::string one_tab = "    ";
    std::string s = tab + "{\n";
    s += (tab + "    " + "sender_id = " + std::to_string(sender_id) + ",\n");
