...
GP22MISAlg::GP22MISAlg(Node *node, int starting_round, double threshold_ratio) {
    ...
    part2_max_num_rounds = part2_num_iterations*part2_num_rounds_per_iteration;
...
void GP22MISAlg::stage_transition() {
    ...
    if (GP22_MIS_stage == GP22MISStage::INITIAL_STAGE) {
        ...
        // do a tricky thing
        node->all_neighbors = std::vector<int>(all_remained_neighbors.begin(), all_remained_neighbors.end());
