...
void GP22MISPart2Alg::set_alg_type()
{
    EV << "GP22MISPart2::set_alg_type()\n";
    alg_type = MIS_ALG;
}
...
void GP22MISPart2Alg::stage_transition()
{
    ...
    current_round_id_in_iteration =
        (current_round_id_in_iteration + 1) % num_rounds_per_iteration;
    if (current_round_id_in_iteration == n_rounds_of_current_phase) {
        ++current_phase_id_in_iteration;
    }
    n_rounds_of_current_phase =
        C * pow(log_d - current_phase_id_in_iteration, 2);
    ...
    if (current_round_id_in_iteration % 2 == 0) {
        ...
        GP22Part2_alg_round_type = GP22Part2_DECLARING_STATUS;
}
...
cMessage *GP22MISPart2Alg::process_message_queue_declaring_mark_round()
{
    ...
    if (MIS_status != UNDECIDED)
        ...
        marked = node->bernoulli(pow(2, current_phase_id_in_iteration) / log_d);
...
bool GP22MISPart2Alg::is_selected() { return (MIS_status == IN_MIS); }