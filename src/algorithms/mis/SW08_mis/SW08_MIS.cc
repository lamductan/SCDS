...
void SW08MISAlg::set_alg_type()
{
    EV << "SW08MIS::set_alg_type()\n";
    alg_type = MIS_ALG;
}
...
SW08MISAlg::SW08MISAlg(Node *node, int starting_round)
{
    ...
    EV << "\tSW08MISAlg::n_competitions_per_phase = "
       << n_competitions_per_phase << '\n';
...
bool SW08MISAlg::is_new_stage()
{
    ...
    for (auto it : neighbors_status) {
        ...
        if (neighbor_status == SW08_RULER ||
            ...
            res = false;
            EV << "Can't start new stage. Neighbor" << it.first << " is "
               << neighbor_status << "\n";
            break;
        }
    }
...
bool SW08MISAlg::is_new_phase()
{
    if (SW08_status == SW08_COMPETITOR) {
        EV << "Can't start new phase. Node" << node->id
           << " is SW08_COMPETITOR\n";
        return false;
    }
    bool res = true;
...
{
    return (SW08_node_status == SW08_DOMINATOR) ||
           (SW08_node_status == SW08_DOMINATED);
}
...
int SW08MISAlg::find_smallest_r()
{
    ...
    for (int neighbor_id : undecided_neighbors) {
        if (is_decided_status(neighbors_status[neighbor_id])) {
            EV_ERROR << "Node" << neighbor_id
                     << " is decided: " << neighbors_status[neighbor_id]
                     << '\n';
        }
...
{
    EV << "\t"
       << "SW08MISAlg::process_message_queue()\n";
...
{
    EV << "SW08MISAlg::process_message_queue_for_exchange_r_round() -- round "
       << current_round_id << "\n";
    ...
    EV << "\t"
       << "Done processing message queue. Now finding smallest r\n";
    if (SW08_status == SW08_COMPETITOR) {
...
{
    EV << "SW08MISAlg::process_message_queue_for_exchange_state_1_round() -- "
          "round "
       << current_round_id << "\n";
    if (SW08_status == SW08_COMPETITOR) {
        ...
        EV << "\t"
           << "r = " << r << '\n';
        for (cMessage *msg : message_queue) {
            SW08MISMessage *SW08_MIS_message =
                dynamic_cast<SW08MISMessage *>(msg);
            int neighbor_id = SW08_MIS_message->getSenderId();
        ...
        if (can_be_dominator) {
            ...
            EV << "can_be_dominator" << '\n';
        } else if (can_be_ruler)
            SW08_status = SW08_RULER;
    }
...
{
    EV << "SW08MISAlg::process_message_queue_for_exchange_state_2_round() -- "
          "round "
       << current_round_id << "\n";
    for (cMessage *msg : message_queue) {
        SW08MISMessage *SW08_MIS_message = dynamic_cast<SW08MISMessage *>(msg);
...
bool SW08MISAlg::is_selected() { return (SW08_status == SW08_DOMINATOR); }

bool SW08MISAlg::is_decided() { return is_decided_status(SW08_status); }
