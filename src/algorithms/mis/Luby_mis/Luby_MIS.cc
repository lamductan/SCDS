...
void LubyMISAlg::set_alg_type()
{
    EV << "LubyMISAlg::set_alg_type()\n";
    alg_type = MIS_ALG;
}
...
{
    EV << "\t"
       << "LubyMISAlg::process_message_queue()\n";
...
cMessage *LubyMISAlg::process_message_queue_for_processing_mark_round()
{
    ...
    if (degree == 0) {
        ...
        need_to_send.clear();
        for (cMessage *msg : message_queue) {
            LubyMISMessage *Luby_MIS_message =
                dynamic_cast<LubyMISMessage *>(msg);
...
bool LubyMISAlg::is_selected() { return (MIS_status == IN_MIS); }