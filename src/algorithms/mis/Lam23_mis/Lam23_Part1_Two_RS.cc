...
void LamTwoRSAlg::set_alg_type()
{
    EV << "LamTwoRSAlg::set_alg_type()\n";
    alg_type = TWO_RS_ALG;
}
...
LamTwoRSAlg::LamTwoRSAlg(Node *node, int starting_round)
{
    ...
    // part2_max_num_rounds = Lam_Two_RS_part2_alg->max_num_rounds; // either
    // use it or the two above lines

    max_num_rounds = part2_starting_round + part2_max_num_rounds;

    EV << "LamTwoRSAlg::part1_starting_round = " << part1_starting_round << ' '
       << Lam_Two_RS_part1_alg->starting_round << '\n';
    EV << "LamTwoRSAlg::part1_2_starting_round = " << part1_2_starting_round
       << ' ' << Lam_Two_RS_part1_2_alg->starting_round << '\n';
    EV << "LamTwoRSAlg::part2_starting_round = " << part2_starting_round << ' '
       << Lam_Two_RS_part2_alg->starting_round << '\n';
...
void LamTwoRSAlg::handle_message(cMessage *msg)
{
    ...
    EV << "LamTwoRSAlg::status after round #" << current_round_id << " = "
       << Lam_Two_RS_status << ": is_decided() = " << is_decided() << '\n';
...
void LamTwoRSAlg::stage_transition()
{
    ...
    if (Lam_Two_RS_stage == LamTwoRSStage::INITIAL_STAGE) {
        ...
        two_rs_cluster_center_id =
            Lam_Two_RS_part1_alg->two_rs_cluster_center_id;
        Lam_Two_RS_part1_2_alg->two_rs_cluster_center_id =
            two_rs_cluster_center_id;
        Lam_Two_RS_part1_2_alg->Lam_Two_RS_status =
            Lam_Two_RS_part1_alg->BGKO22_Two_RS_status;
        Lam_Two_RS_part1_2_alg->all_remained_neighbors =
            Lam_Two_RS_part1_alg->all_remained_neighbors;
    } else if (current_round_id == part2_starting_round) {
        if (Lam_Two_RS_status != LAM_TWO_RS_UNDECIDED)
            Lam_Two_RS_stage = LamTwoRSStage::END_STAGE;
        else {
...
void LamTwoRSAlg::call_handle_message(IAlgNode *alg, cMessage *msg)
{
    ...
    case LamTwoRSStage::PART1_2: {
        LamInformingNeighborsAlg *alg_1_2 =
            dynamic_cast<LamInformingNeighborsAlg *>(alg);
    ...
    if (Lam_Two_RS_status == LAM_TWO_RS_CLUSTER_CENTER) {
        ...
        if (previous_MIS_status == UNDECIDED) {
            ...
            EV << "LamTwoRSAlg::call_handle_message() --- decided_round = "
               << decided_round << '\n';
        }
    ...
    EV << "LamTwoRSAlg::call_handle_message: "
       << "Lam_Two_RS_status = " << Lam_Two_RS_status
