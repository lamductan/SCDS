...
void BGKO22TwoRSAlg::set_alg_type() { EV << "BGKO22TwoRSAlg::set_alg_type()\n"; alg_type = TWO_RS_ALG; }
...
void BGKO22TwoRSAlg::stage_transition() {
    ...
    } else if (BGKO22_Two_RS_alg_round_type == BGKO22_TWO_RS_INFORMING_STATUS_1) {
        BGKO22_Two_RS_alg_round_type = BGKO22_TWO_RS_INFORMING_STATUS_2;
    } else if (BGKO22_Two_RS_alg_round_type == BGKO22_TWO_RS_INFORMING_STATUS_2) {
...
cMessage *BGKO22TwoRSAlg::process_message_queue_for_generate_mark_round() {
    ...
    for(cMessage *msg : message_queue) {
        ...
        LamTwoRSNodeStatus BGKO22_Two_RS_neighbor_status = Lam_MIS_message->getTwoRSStatus();
        if (BGKO22_Two_RS_neighbor_status != LAM_TWO_RS_UNDECIDED) all_remained_neighbors.erase(neighbor_id);
        if (BGKO22_Two_RS_neighbor_status == LAM_TWO_RS_1_HOP) {
            if (BGKO22_Two_RS_status == LAM_TWO_RS_UNDECIDED) {
                ...
                //parent = neighbor_id;
                two_rs_cluster_center_id = Lam_MIS_message->getClusterCenterId();
            }
        }
    }
...
cMessage *BGKO22TwoRSAlg::process_message_queue_for_informing_status_2_round() {
    ...
    for(cMessage *msg : message_queue) {
        ...
        EV << "\t\t\t" << "neighbor_id = " << neighbor_id
           << ", BGKO22_Two_RS_neighbor_status = " << BGKO22_Two_RS_neighbor_status
           << ", BGKO22_neighbor_cluster_center_id = " << Lam_MIS_message->getClusterCenterId() << '\n';
...
void BGKO22TwoRSAlg::record_decided_round() { //only record node that is cluster center and 1 hop
    if (BGKO22_Two_RS_previous_status == LAM_TWO_RS_UNDECIDED && BGKO22_Two_RS_status == LAM_TWO_RS_CLUSTER_CENTER) {
        decided_round = current_round_id;
    }
    else if (BGKO22_Two_RS_previous_status == LAM_TWO_RS_UNDECIDED && BGKO22_Two_RS_status == LAM_TWO_RS_1_HOP) {
        decided_round = current_round_id;
    }
}