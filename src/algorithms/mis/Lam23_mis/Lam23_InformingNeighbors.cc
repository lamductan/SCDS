...
void LamInformingNeighborsAlg::set_alg_type()
{
    EV << "LamInformingNeighborsAlg::set_alg_type()\n";
    alg_type = MIS_ALG;
}

LamInformingNeighborsAlg::LamInformingNeighborsAlg(Node *node,
                                                   int starting_round)
{
...
cMessage *LamInformingNeighborsAlg::process_message_queue()
{
    ...
    for (cMessage *msg : message_queue) {
        ...
        if (neighbor_color != -1)
            neighbor_colors.insert({ neighbor_color, neighbor_id });
        if (Lam_Two_RS_neighbor_status == LAM_TWO_RS_CLUSTER_CENTER ||
            Lam_Two_RS_neighbor_status == LAM_TWO_RS_1_HOP)
...
bool LamInformingNeighborsAlg::is_selected() { return false; }
