...
void NetworkAlg::set_alg_type()
{
    ...
    EV << "NetworkAlg::set_alg_type: "
       << "alg_name = " << alg_name << ", alg_type = " << alg_type
       << ", n_nodes = " << n_nodes << '\n';
}
...
void NetworkAlg::handle_message(cMessage *msg)
{
    if (msg->isSelfMessage() && msg->getKind() == START_ROUND_MESSAGE) {
        ...
        if (current_round_id == max_num_rounds) {
            EV << "NetworkAlg::handle_message() -- FINISH at Round "
               << current_round_id << "\n";
        } else {
            EV << "NetworkAlg::handle_message() -- START_ROUND_MESSAGE -- "
                  "Round "
               << current_round_id << "\n";
            ...
            network->scheduleAt(
                current_round_id,
                new cMessage("Start Rounds", START_ROUND_MESSAGE));
        }
    }
    delete msg;
}
...
{
    EV << "NetworkAlg::send_synchronized_message(), type = "
       << synchronized_message_type << '\n';
    SynchronizedMessage *synchronized_message =
        new SynchronizedMessage("synchronized");
